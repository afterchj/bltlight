package com.tpadsz.after.viewresolver;
/*
...
@Service("jsonView")
public class MyEncryptedMappingJacksonJsonView extends AbstractView {
	 ...
	 */
	public MyEncryptedMappingJacksonJsonView() {
	...
	protected void writeContent(OutputStream stream, Object value, boolean prefixJson) throws IOException {
		...
		Object encrypted = valueMap.remove("encrypted");
		JsonGenerator generator = this.objectMapper.getJsonFactory().createJsonGenerator(baos, this.encoding);
		...
		if(encrypted != null && (boolean)encrypted){
			bytes = URLEncoder.encode(Encodes.encodeBase64(DESCoder.encrypt(bytes)),"utf-8").getBytes();
		}
//		if(gzip != null && (Boolean)gzip){
//			String compress = GzipUtil.compressStringToString(new String(bytes,"ISO-8859-1"));
//			bytes = compress.getBytes();
//		}
		try {
			stream.write(bytes);
		} catch (Exception e) {
			logger.error("MappingJacksonJsonView writeContent error.", e);
		}
	}
